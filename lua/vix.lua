...
--- Map a new operator.
--
-- Sets up a mapping in normal, vixual and operator pending mode.
...
-- @usage
-- vix:operator_new("gq", function(file, range, pos)
-- 	local status, out, err = vix:pipe(file, range, "fmt")
-- 	if status ~= 0 then
-- 		vix:info(err)
-- 	else
-- 		file:delete(range)
...
--
vix.operator_new = function(vix, key, operator, help)
	local id = vix:operator_register(operator)
	local binding = function()
		vix:operator(id)
	end
	vix:map(vix.modes.NORMAL, key, binding, help)
	vix:map(vix.modes.VISUAL, key, binding, help)
	vix:map(vix.modes.OPERATOR_PENDING, key, binding, help)
	return true
end
...
-- @tparam[opt] string help the single line help text as displayed in `:help`
-- @usage
-- vix:motion_new("<C-l>", function(win, pos)
-- 	return pos+1
-- end, "Advance to next byte")
--
vix.motion_new = function(vix, key, motion, help)
	local id = vix:motion_register(motion)
	local binding = function()
		vix:motion(id)
	end
	vix:map(vix.modes.NORMAL, key, binding, help)
...
--
-- Sets up a mapping in vixual and operator pending mode.
...
-- @tparam[opt] string help the single line help text as displayed in `:help`
-- @usage
-- vix:textobject_new("<C-l>", function(win, pos)
-- 	return pos, pos+1
-- end, "Single byte text object")
--
vix.textobject_new = function(vix, key, textobject, help)
	local id = vix:textobject_register(textobject)
	local binding = function()
		vix:textobject(id)
	end
	vix:map(vix.modes.VISUAL, key, binding, help)
...
vix.module_exist = function(_, name)
...
if not vix:module_exist('lpeg') then
	vix:info('WARNING: could not find lpeg module')
elseif not vix:module_exist('lexer') then
	vix:info('WARNING: could not find lexer module')
else
	vix.lexers = require('lexer')
	...
	local lexers = {}
	local load_lexer = vix.lexers.load
	vix.lexers.load = function (name, alt_name, cache)
	...
	pcall(function() vix.lpeg = require('lpeg') end)
end
...
-- @usage
-- vix.events.subscribe(vix.events.FILE_SAVE_PRE, function(file, path)
-- 	-- do something useful
-- 	return true
-- end)
...
vix.types.window.set_syntax = function(win, syntax)
	...
	win:style_define(win.STYLE_TAB, lexers.STYLE_TAB or lexers.STYLE_STATUS or '')
	win:style_define(win.STYLE_TAB_FOCUSED, lexers.STYLE_TAB_FOCUSED or lexers.STYLE_STATUS_FOCUSED or '')
...
vix.types.file.match_at = function(file, pattern, pos, horizon)
