...
--[[
	...
	vix.ftdetect.extensions.sh = "bash"
	vix.ftdetect.filenames.makefile = "makefile"
	vix.ftdetect.mimes["plain"] = "text"
	vix.ftdetect.utilities.python3 = "python" -- for shebang
	vix.ftdetect.data_patterns["^#!%s*.-[/ ]awk"] = "awk"

	If you do not want an exact match, you can use the filetypes table below
	See FINDERS
		table.insert(vix.ftdetect.cmake.name, "%.cmake%.in")

	Actions can be performed via the filetype table as well, assuming extensions is sh then
		table.insert( vix.ftdetect.filetypes.bash.cmd, "set shell /bin/bash")
...
--[[
	...
	ACTIONS: These run if a finder succeeds
		...
		cmd: ipairs = { "command" } -- run list of commands (via vix:command(command))
