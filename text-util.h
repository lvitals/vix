...
VIX_INTERNAL Filerange text_range_union(const Filerange*, const Filerange*);
/* get intersection of two ranges */
VIX_INTERNAL Filerange text_range_intersect(const Filerange*, const Filerange*);
/* create new range [min(a,b), max(a,b)] */
VIX_INTERNAL Filerange text_range_new(size_t a, size_t b);
/* test whether two ranges are equal */
VIX_INTERNAL bool text_range_equal(const Filerange*, const Filerange*);
/* test whether two ranges overlap */
VIX_INTERNAL bool text_range_overlap(const Filerange*, const Filerange*);
/* test whether a given position is within a certain range */
VIX_INTERNAL bool text_range_contains(const Filerange*, size_t pos);
/* count the number of graphemes in data */
VIX_INTERNAL int text_char_count(const char *data, size_t len);
/* get the approximate display width of data */
VIX_INTERNAL int text_string_width(const char *data, size_t len);
