Tests for vix - structural regular expression support
-----------------------------------------------------

We test the structural regular expression implementation by supplying
the same command to both vix and [sam](http://sam.cat-v.org/). More
...
implementation in sam and vix which need to be taken into account
when writing tests:

 * _missing commands_: vix does currently deliberately not implement
 ...
 * _different semantics_: some language constructs have slightly
   ...
   while in `vix` they stop at the start/end of the file.
 ...
 * _changes need to be in sequence_: sam will reject changes which are not
   ...
   In contrast vix only requires that the changes are non-overlapping.
 ...
 * _spurious white spaces_: in sam an empty line/command affects the
   ...
   the whole line. However in vix we ignore all white space between
...
vix behavior might be changed in the future.
 ...
 * `test.cmd` a file containing the structural regular expression
    command as you would type it in vix at the `:`-prompt or pass to
    `ssam(1)`.
