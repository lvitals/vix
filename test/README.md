Testing Infrastructure for Vix
------------------------------

This repository contains testing infrastructure for the
[vix editor](https://github.com/username/vix).

There exist 5 different kinds of tests:

 * `core` are C unit tests for core data structures used by vix
 * `fuzz` infrastructure for automated fuzzing
 * `vim` tests vim compatibility
 * `sam` tests sam compatibility of the command language
 * `vix` contains tests for vix specific behavior/features
 * `lua` contains tests for the vix specific lua api
 ...
 * Test systematically and strive for high test coverage.
   ...
   [achieved code coverage](https://codecov.io/gh/martanne/vix). Ideally
