...
struct CommandDef {
	...
	VIX_HELP_DECL(const char *help;)     /* short, one-line help text */
	enum {
...
/* sam commands */
static bool cmd_insert(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_append(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_change(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_delete(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_guard(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_extract(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_select(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_print(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_files(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_pipein(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_pipeout(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_filter(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_launch(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_substitute(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_write(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_read(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_edit(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_quit(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_cd(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
/* vi(m) commands */
static bool cmd_set(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_open(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_qall(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_split(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_vsplit(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_new(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_vnew(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_wq(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_earlier_later(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_help(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_map(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_unmap(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_langmap(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);
static bool cmd_user(Vix*, Win*, Command*, const char *argv[], Selection*, Filerange*);

static const CommandDef cmds[] = {
	//      name            help
	//      flags, default command, implementation
	{
		"a",            VIX_HELP("Append text after range")
		CMD_TEXT, NULL, cmd_append
	}, {
		"c",            VIX_HELP("Change text in range")
		CMD_TEXT, NULL, cmd_change
	}, {
		"d",            VIX_HELP("Delete text in range")
		CMD_NONE, NULL, cmd_delete
	}, {
		"g",            VIX_HELP("If range contains regexp, run command")
		CMD_COUNT|CMD_REGEX|CMD_CMD, "p", cmd_guard
	}, {
		"i",            VIX_HELP("Insert text before range")
		CMD_TEXT, NULL, cmd_insert
	}, {
		"p",            VIX_HELP("Create selection covering range")
		CMD_NONE, NULL, cmd_print
	}, {
		"s",            VIX_HELP("Substitute: use x/pattern/ c/replacement/ instead")
		CMD_SHELL|CMD_ADDRESS_LINE, NULL, cmd_substitute
	}, {
		"v",            VIX_HELP("If range does not contain regexp, run command")
		CMD_COUNT|CMD_REGEX|CMD_CMD, "p", cmd_guard
	}, {
		"x",            VIX_HELP("Set range and run command on each match")
		CMD_CMD|CMD_REGEX|CMD_REGEX_DEFAULT|CMD_ADDRESS_ALL_1CURSOR|CMD_LOOP, "p", cmd_extract
	}, {
		"y",            VIX_HELP("As `x` but select unmatched text")
		CMD_CMD|CMD_REGEX|CMD_ADDRESS_ALL_1CURSOR|CMD_LOOP, "p", cmd_extract
	}, {
		"X",            VIX_HELP("Run command on files whose name matches")
		CMD_CMD|CMD_REGEX|CMD_REGEX_DEFAULT|CMD_ADDRESS_NONE|CMD_ONCE, NULL, cmd_files
	}, {
		"Y",            VIX_HELP("As `X` but select unmatched files")
		CMD_CMD|CMD_REGEX|CMD_ADDRESS_NONE|CMD_ONCE, NULL, cmd_files
	}, {
		">",            VIX_HELP("Send range to stdin of command")
		CMD_SHELL|CMD_ADDRESS_LINE, NULL, cmd_pipeout
	}, {
		"<",            VIX_HELP("Replace range by stdout of command")
		CMD_SHELL|CMD_ADDRESS_POS, NULL, cmd_pipein
	}, {
		"|",            VIX_HELP("Pipe range through command")
		CMD_SHELL, NULL, cmd_filter
	}, {
		"!",            VIX_HELP("Run the command")
		CMD_SHELL|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_launch
	}, {
		"w",            VIX_HELP("Write range to named file")
		CMD_ARGV|CMD_FORCE|CMD_ONCE|CMD_ADDRESS_ALL, NULL, cmd_write
	}, {
		"r",            VIX_HELP("Replace range by contents of file")
		CMD_ARGV|CMD_ADDRESS_AFTER, NULL, cmd_read
	}, {
		"{",            VIX_HELP("Start of command group")
		CMD_GROUP, NULL, NULL
	}, {
		"}",            VIX_HELP("End of command group" )
		CMD_NONE, NULL, NULL
	}, {
		...
		"cd",           VIX_HELP("Change directory")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_cd
	},
	/* vi(m) related commands */
	{
		"help",         VIX_HELP("Show this help")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_help
	}, {
		"map",          VIX_HELP("Map key binding `:map <mode> <lhs> <rhs>`")
		CMD_ARGV|CMD_FORCE|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_map
	}, {
		"map-window",   VIX_HELP("As `map` but window local")
		CMD_ARGV|CMD_FORCE|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_map
	}, {
		"unmap",        VIX_HELP("Unmap key binding `:unmap <mode> <lhs>`")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_unmap
	}, {
		"unmap-window", VIX_HELP("As `unmap` but window local")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_unmap
	}, {
		"langmap",      VIX_HELP("Map keyboard layout `:langmap <locale-keys> <latin-keys>`")
		CMD_ARGV|CMD_FORCE|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_langmap
	}, {
		"new",          VIX_HELP("Create new window")
		...
		"set",          VIX_HELP("Set option")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_set
	}, {
		"split",        VIX_HELP("Horizontally split window")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_split
	}, {
		"vnew",         VIX_HELP("As `:new` but split vertically")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_vnew
	}, {
		"vsplit",       VIX_HELP("Vertically split window")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_vsplit
	}, {
		"wq",           VIX_HELP("Write file and quit")
		CMD_ARGV|CMD_FORCE|CMD_ONCE|CMD_ADDRESS_ALL|CMD_DESTRUCTIVE, NULL, cmd_wq
	}, {
		"earlier",      VIX_HELP("Go to older text state")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_earlier_later
	}, {
		"later",        VIX_HELP("Go to newer text state")
		CMD_ARGV|CMD_ONCE|CMD_ADDRESS_NONE, NULL, cmd_earlier_later
	},
	{ NULL, VIX_HELP(NULL) CMD_NONE, NULL, NULL },
};

static const CommandDef cmddef_select = {
	NULL, VIX_HELP(NULL) CMD_NONE, NULL, cmd_select
};

/* :set command options */
typedef struct {
	const char *names[3];            /* name and optional alias */
	enum VixOption flags;            /* option type, etc. */
	VIX_HELP_DECL(const char *help;) /* short, one line help text */
	VixOptionFunction *func;         /* option handler, NULL for builtins */
...
static const OptionDef options[] = {
	[OPTION_SHELL] = {
		{ "shell" },
		VIX_OPTION_TYPE_STRING,
		VIX_HELP("Shell to use for external commands (default: $SHELL, /etc/passwd, /bin/sh)")
	},
	[OPTION_ESCDELAY] = {
		{ "escdelay" },
		VIX_OPTION_TYPE_NUMBER,
		VIX_HELP("Milliseconds to wait to distinguish <Escape> from terminal escape sequences")
	},
	[OPTION_AUTOINDENT] = {
		{ "autoindent", "ai" },
		VIX_OPTION_TYPE_BOOL,
		VIX_HELP("Copy leading white space from previous line")
	},
	[OPTION_EXPANDTAB] = {
		{ "expandtab", "et" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Replace entered <Tab> with `tabwidth` spaces")
	},
	[OPTION_TABWIDTH] = {
		{ "tabwidth", "tw" },
		VIX_OPTION_TYPE_NUMBER|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Number of spaces to display (and insert if `expandtab` is enabled) for a tab")
	},
	[OPTION_SHOW_SPACES] = {
		{ "showspaces" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Display replacement symbol instead of a space")
	},
	[OPTION_SHOW_TABS] = {
		{ "showtabs" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Display replacement symbol for tabs")
	},
	[OPTION_SHOW_NEWLINES] = {
		{ "shownewlines" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Display replacement symbol for newlines")
	},
	[OPTION_SHOW_EOF] = {
		{ "showeof" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Display replacement symbol for lines after the end of the file")
	},
	[OPTION_STATUSBAR] = {
		{ "statusbar", "sb" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Display status bar")
	},
	[OPTION_NUMBER] = {
		{ "numbers", "nu" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Display absolute line numbers")
	},
	[OPTION_NUMBER_RELATIVE] = {
		{ "relativenumbers", "rnu" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Display relative line numbers")
	},
	[OPTION_CURSOR_LINE] = {
		{ "cursorline", "cul" },
		VIX_OPTION_TYPE_BOOL|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Highlight current cursor line")
	},
	[OPTION_COLOR_COLUMN] = {
		{ "colorcolumn", "cc" },
		VIX_OPTION_TYPE_NUMBER|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Highlight a fixed column")
	},
	[OPTION_SAVE_METHOD] = {
		{ "savemethod" },
		VIX_OPTION_TYPE_STRING|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Save method to use for current file 'auto', 'atomic' or 'inplace'")
	},
	[OPTION_LOAD_METHOD] = {
		{ "loadmethod" },
		VIX_OPTION_TYPE_STRING,
		VIX_HELP("How to load existing files 'auto', 'read' or 'mmap'")
	},
	[OPTION_CHANGE_256COLORS] = {
		{ "change256colors" },
		VIX_OPTION_TYPE_BOOL,
		VIX_HELP("Change 256 color palette to support 24bit colors")
	},
	[OPTION_LAYOUT] = {
		{ "layout" },
		VIX_OPTION_TYPE_STRING,
		VIX_HELP("Vertical or horizontal window layout")
	},
	[OPTION_IGNORECASE] = {
		{ "ignorecase", "ic" },
		VIX_OPTION_TYPE_BOOL,
		VIX_HELP("Ignore case when searching")
	},
	[OPTION_BREAKAT] = {
		{ "breakat", "brk" },
		VIX_OPTION_TYPE_STRING|VIX_OPTION_NEED_WINDOW,
		VIX_HELP("Characters which might cause a word wrap")
	},
	[OPTION_WRAP_COLUMN] = {
		{ "wrapcolumn", "wc" },
		VIX_OPTION_TYPE_NUMBER|VIX_OPTION_NEED_WINDOW,
...
static char *parse_shellcmd(Vix *vix, const char **s) {
	skip_spaces(s);
	char *cmd = parse_until(s, "\n", NULL, false);
	if (!cmd) {
		const char *last_cmd = register_get(vix, &vix->registers[VIX_REG_SHELL], NULL);
		return last_cmd ? strdup(last_cmd) : NULL;
	}
	register_put0(vix, &vix->registers[VIX_REG_SHELL], cmd);
	return cmd;
}
...
static Address *address_parse_simple(Vix *vix, const char **s, enum SamError *err) {
	...
	case '\'':
		(*s)++;
		if ((addr.number = vix_mark_from(vix, **s)) == VIX_MARK_INVALID) {
			*err = SAM_ERR_MARK;
			return NULL;
		}
		(*s)++;
	...
	if ((addr.right = address_parse_simple(vix, s, err))) {
...
static Address *address_parse_compound(Vix *vix, const char **s, enum SamError *err) {
	Address addr = { 0 }, *left = address_parse_simple(vix, s, err), *right = NULL;
	skip_spaces(s);
	addr.type = **s;
	switch (addr.type) {
	case ',': /* a1,a2 */
	case ';': /* a1;a2 */
		(*s)++;
		right = address_parse_compound(vix, s, err);
...
static const CommandDef *command_lookup(Vix *vix, const char *name) {
	return map_closest(vix->cmds, name);
}

static Command *command_parse(Vix *vix, const char **s, enum SamError *err) {
	if (!**s) {
		*err = SAM_ERR_COMMAND;
		return NULL;
	}
	...
	if (cmddef->flags & CMD_SHELL && !(cmd->argv[1] = parse_shellcmd(vix, s))) {
		*err = SAM_ERR_SHELL;
		goto fail;
	}
...
static Command *sam_parse(Vix *vix, const char *cmd, enum SamError *err) {
	vix->nesting_level = 0;
	const char **s = &cmd;
...
static bool sam_execute(Vix *vix, Win *win, Command *cmd, Selection *sel, Filerange *range) {
	...
	default:
		ret = cmd->cmddef->func(vix, win, cmd, cmd->argv, sel, range);
		break;
	}
	return ret;
}
...
enum SamError sam_cmd(Vix *vix, const char *s) {
	...
	size_t primary_pos = vix->win ? view_cursor_get(&vix->win->view) : EPOS;
		...
		for (SamChange *c = t->changes; c; c = c->next) {
			...
			if (c->type & TRANSCRIPT_INSERT) {
				for (int i = 0; i < c->count; i++) {
					text_insert(vix, file->text, c->range.start, c->data, c->len);
					delta += c->len;
				}
	...
	if (vix->win) {
		...
		bool completed = true;
		for (Selection *s = view_selections(&vix->win->view); s; s = view_selections_next(s)) {
			if (s->anchored) {
				completed = false;
				break;
			}
		}
		vix_mode_switch(vix, completed ? VIX_MODE_NORMAL : VIX_MODE_VISUAL);
	}
	command_free(cmd);
	return err;
}
...
Buffer text(Vix *vix, const char *text) {
	Buffer buf = {0};
	for (size_t len = strcspn(text, "\\&"); *text; len = strcspn(++text, "\\&")) {
		buffer_append(&buf, text, len);
		text += len;
		enum VixRegister regid = VIX_REG_INVALID;
		switch (text[0]) {
		case '&':
			regid = VIX_REG_AMPERSAND;
			break;
		case '\\':
			if ('1' <= text[1] && text[1] <= '9') {
				regid = VIX_REG_1 + text[1] - '1';
				text++;
		...
		const char *data;
		size_t reglen = 0;
		if (regid != VIX_REG_INVALID) {
			data = register_get(vix, &vix->registers[regid], &reglen);
		} else {
			data = text;
			reglen = 1;
		}
...
static bool cmd_delete(Vix *vix, Win *win, Command *cmd, const char *argv[], Selection *sel, Filerange *range) {
...
static int extract(Vix *vix, Win *win, Command *cmd, const char *argv[], Selection *sel, Filerange *range, bool simulate) {
	bool ret = true;
	int count = 0;
			...
			if (text_range_valid(&r)) {
				if (found) {
					for (size_t i = 0; i < nsub; i++) {
						Register *reg = &vix->registers[VIX_REG_AMPERSAND+i];
						register_put_range(vix, reg, txt, &match[i]);
					}
...
static bool cmd_files(Vix *vix, Win *win, Command *cmd, const char *argv[], Selection *sel, Filerange *range) {
	bool ret = true;
	for (Win *wn, *w = vix->windows; w; w = wn) {
		...
		if (match ^ (argv[0][0] == 'Y')) {
			...
			ret &= sam_execute(vix, w, cmd->cmd, NULL, &def);
		}
	}
	return ret;
}

static bool cmd_substitute(Vix *vix, Win *win, Command *cmd, const char *argv[], Selection *sel, Filerange *range) {
	vix_info_show(vix, "Use :x/pattern/ c/replacement/ instead");
	return false;
}
...
static bool cmd_write(Vix *vix, Win *win, Command *cmd, const char *argv[], Selection *sel, Filerange *r) {
	...
	if (!filename) {
		...
		if (!vix_event_emit(vix, VIX_EVENT_FILE_SAVE_PRE, file, (char*)NULL) && cmd->flags != '!') {
			vix_info_show(vix, "Rejected write to stdout by pre-save hook");
			return false;
		}
		...
		vix_event_emit(vix, VIX_EVENT_FILE_SAVE_POST, file, (char*)NULL);
		return true;
	}

	if (!argv[1] && cmd->flags != '!') {
		if (vix->mode->visual) {
			vix_info_show(vix, "WARNING: file will be reduced to active selection");
			return false;
		}
		if (!write_entire_file) {
			vix_info_show(vix, "WARNING: file will be reduced to provided range");
			return false;
		}
	}
	...
	for (const char **name = argv[1] ? &argv[1] : (const char*[]){ filename, NULL }; *name; name++) {
		...
		if (!path) {
			vix_info_show(vix, "Failed to resolve path '%s': %s", *name, strerror(errno));
			return false;
		}
		...
		if (cmd->flags != '!') {
			if (same_file && file->stat.st_mtime && file->stat.st_mtime < meta.st_mtime) {
				vix_info_show(vix, "WARNING: file has been changed since reading it");
				goto err;
			}
			if (existing_file && !same_file) {
				vix_info_show(vix, "WARNING: file exists");
				goto err;
			}
			/* Check if the destination file exists and is read-only. This
			 * is needed because vix' default save method uses mkstemp and
			...
			if (existing_file && !(meta.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH))) {
				vix_info_show(vix, "WARNING: file is read-only");
				goto err;
			}
		}

		if (!vix_event_emit(vix, VIX_EVENT_FILE_SAVE_PRE, file, path) && cmd->flags != '!') {
			vix_info_show(vix, "Rejected write to `%s' by pre-save hook", path);
			goto err;
		}
		/* a pre-save hook may have changed the text; need to re-take the range */
		if (write_entire_file) {
		...
		if (!text_save_begin(&ctx)) {
			...
			vix_info_show(vix, "Can't write `%s': %s", path, msg);
			goto err;
		}
		...
		if (failure) {
			vix_info_show(vix, "Can't write `%s': %s", path, strerror(errno));
			goto err;
		}
...
static bool cmd_filter(Vix *vix, Win *win, Command *cmd, const char *argv[], Selection *sel, Filerange *range) {
	...
	int status = vix_pipe(vix, win->file, range, &argv[1], &bufout, read_into_buffer, &buferr,
	...
	return !vix->interrupted && status == 0;
}

static bool cmd_launch(Vix *vix, Win *win, Command *cmd, const char *argv[], Selection *sel, Filerange *range) {
	Filerange invalid = text_range_new(sel ? view_cursors_pos(sel) : range->start, EPOS);
	return cmd_filter(vix, win, cmd, argv, sel, &invalid);
}
