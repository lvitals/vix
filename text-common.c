...
static bool text_vprintf(Vix *vix, Text *txt, size_t pos, const char *format, va_list ap)
{
	va_list ap_save;
	va_copy(ap_save, ap);
	...
	bool ret = buf && (vsnprintf(buf, len+1, format, ap_save) == len) && text_insert(vix, txt, pos, buf, len);
	free(buf);
	va_end(ap_save);
	return ret;
}

bool text_appendf(Vix *vix, Text *txt, const char *format, ...)
{
	va_list ap;
	va_start(ap, format);
	bool ret = text_vprintf(vix, txt, text_size(txt), format, ap);
	va_end(ap);
	return ret;
}
