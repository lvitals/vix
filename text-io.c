...
Text *text_load(Vix *vix, const char *filename)
{
	return text_load_method(vix, filename, TEXT_LOAD_AUTO);
}

Text *text_load_method(Vix *vix, const char *filename, enum TextLoadMethod method)
{
	return text_loadat_method(vix, AT_FDCWD, filename, method);
}
...
/* Create a new file named `.filename.vix.XXXXXX` (where `XXXXXX` is a
