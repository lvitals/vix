...
VIX_INTERNAL Filerange text_object_entire(Text*, size_t pos);
/* word which happens to be at pos without any neighbouring white spaces */
VIX_INTERNAL Filerange text_object_word(Text*, size_t pos);
...
VIX_INTERNAL Filerange text_object_word_outer(Text*, size_t pos);
/* find next occurrence of `word' (as word not substring) in forward/backward direction */
VIX_INTERNAL Filerange text_object_word_find_next(Text*, size_t pos, const char *word);
VIX_INTERNAL Filerange text_object_word_find_prev(Text*, size_t pos, const char *word);
/* find next occurrence of a literal string (not regex) in forward/backward direction */
VIX_INTERNAL Filerange text_object_find_next(Text *txt, size_t pos, const char *search);
VIX_INTERNAL Filerange text_object_find_prev(Text *txt, size_t pos, const char *search);
/* same semantics as above but for a longword (i.e. delimited by white spaces) */
VIX_INTERNAL Filerange text_object_longword(Text*, size_t pos);
VIX_INTERNAL Filerange text_object_longword_outer(Text*, size_t pos);

VIX_INTERNAL Filerange text_object_line(Text*, size_t pos);
VIX_INTERNAL Filerange text_object_line_inner(Text*, size_t pos);
VIX_INTERNAL Filerange text_object_sentence(Text*, size_t pos);
VIX_INTERNAL Filerange text_object_paragraph(Text*, size_t pos);
VIX_INTERNAL Filerange text_object_paragraph_outer(Text*, size_t pos);
